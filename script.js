...
const logoWidth = 60; // Ajuste conforme o tamanho da sua logo
const logoHeight = 40; // Ajuste conforme o tamanho da sua logo
const logoX = -2; // Posição X da logo
const logoY = startY - 48; // Posição Y da logo
...
// Adicione o texto ao lado da logo
    ...
    doc.setFontSize(16);
    doc.text('Dados de Cadastro', pageWidth / 2, startY + -25 , null, null, 'center');
...
doc.text(`${dados.data.getDate().toString().padStart(2, '0')}/${(dados.data.getMonth() + 1).toString().padStart(2, '0')}/${dados.data.getFullYear()}`, pageWidth / 2 - 55 + 2, startY + 90); // Alinhado à esquerda
    ...
    const assinaturaY = startY + 160;

    // Linha para assinatura do Setor Responsável
    doc.line(x1, assinaturaY, x1 + assinaturaWidth, assinaturaY);
    doc.text('Ass. do Setor Responsável', x1 + assinaturaWidth / 2, assinaturaY + 10, null, null, 'center');

    // Linha para assinatura do Colaborador
    doc.line(x1 + assinaturaWidth + gap, assinaturaY, x1 + 2 * assinaturaWidth + gap, assinaturaY);
    doc.text('Ass. do Colaborador', x1 + assinaturaWidth + gap + assinaturaWidth / 2, assinaturaY + 10, null, null, 'center');

    // Linha para assinatura do Gestor
    doc.line(x1 + 2 * (assinaturaWidth + gap), assinaturaY, x1 + 3 * assinaturaWidth + 2 * gap, assinaturaY);
    doc.text('Ass. do Gestor', x1 + 2 * assinaturaWidth + 2 * gap + assinaturaWidth / 2, assinaturaY + 10, null, null, 'center');

    // Adicione a frase de boas-vindas abaixo das assinaturas
const welcomeMessageY = assinaturaY + 60; // Ajuste a posição vertical do texto de boas-vindas
const welcomeMessage = 'Bem-vindos à empresa! Passo 1: Faça login no notebook usando o logon e a senha provisória fornecidos ( No primeiro acesso, você precisará criar uma senha pessoal ). Passo 2: faça o mesmo no Email, utilize seu email e sua senha provisória. Passo 3: Informações sobre seus outros acessos serão enviados por email.';

doc.setFontSize(12);
doc.text(welcomeMessage, 10, welcomeMessageY, { maxWidth: pageWidth - 20 });
const lines = doc.splitTextToSize(welcomeMessage, pageWidth - 20);

// Adicione o texto dividido no PDF
doc.text(lines, 10, welcomeMessageY);
